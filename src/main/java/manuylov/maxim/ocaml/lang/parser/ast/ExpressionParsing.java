...
class ExpressionParsing extends Parsing
{
	...
	private static boolean tryParseEqLtGtVbarAmpDollarOperatorExpression(@NotNull final PsiBuilder builder)
	{
		...
		while(tryParseInfixOperator(builder, TokenSet.create(OCamlTokenTypes.AMP, OCamlTokenTypes.AMP_AMP, OCamlTokenTypes.VBAR_VBAR),
				Collections.<String>emptySet(), TokenSet.EMPTY, Keywords.EQ, Keywords.LT, Keywords.GT, Keywords.VBAR, Keywords.AMP, Keywords.DOLLAR))
		{
			...
			@NotNull final Set<String> ignoredFirstChars, @NotNull final TokenSet operators, @NotNull final String... firstChars)
	{
		...
		if(ignoredOperators.contains(builder.getTokenType()))
		{
			return false;
		}
		...
		if(!startsWithFirstChars && !operators.contains(builder.getTokenType()))
		{
			return false;
		}
	...
	private static boolean tryParseMultExpression(@NotNull final PsiBuilder builder)
	{
		...
		while(tryParseInfixOperator(builder, TokenSet.EMPTY, Collections.singleton(Keywords.POWER), TokenSet.create(OCamlTokenTypes.MOD_KEYWORD,
				OCamlTokenTypes.LAND_KEYWORD, OCamlTokenTypes.LOR_KEYWORD, OCamlTokenTypes.LXOR_KEYWORD), Keywords.MULT, Keywords.DIV, Keywords.PERCENT))
		{
	...
	private static boolean tryParsePowerExpression(@NotNull final PsiBuilder builder)
	{
		...
		while(tryParseInfixOperator(builder, TokenSet.EMPTY, Collections.<String>emptySet(), TokenSet.create(OCamlTokenTypes.LSL_KEYWORD,
				OCamlTokenTypes.LSR_KEYWORD, OCamlTokenTypes.ASR_KEYWORD), Keywords.POWER))
		{
			binaryExpressionMarker.mark();
