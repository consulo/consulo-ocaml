...
class NameParsing extends Parsing
{
	...
	private static void doParseLabelNameWithColon(final PsiBuilder builder, final Runnable afterColonProcessing, final boolean colonIsMandatory,
			final boolean isDefinition)
	{
	...
	public static boolean tryParseConstant(@NotNull final PsiBuilder builder, @NotNull final NameType type)
	{
		...
		if(!tryParseCharLiteral(builder) && !ignore(builder, TokenSet.create(OCamlTokenTypes.INTEGER_LITERAL, OCamlTokenTypes.FLOAT_LITERAL,
