...
public class OCamlCompletionContributor extends CompletionContributor
{
	...
	public OCamlCompletionContributor()
	{
		...
		{
			extend(CompletionType.BASIC, OCAML_ELEMENT.and(isPossibleHere(keyword)), createCompletionProvider(TailType.SPACE, keyword));
		}
	...
	private static Set<String> collectAllKeywords()
	{
		...
		{
			if(field.isAnnotationPresent(DoNotSuggestInCompletionVariants.class))
			{
				continue;
			}
			try
			...
			final boolean parseWholeFile)
	{
		...
		else
		{
			...
			offsetInStatement = prevStatementLength + (statement == null ? 0 : getStartOffset(parameters.getPosition()) - getStartOffset(statement));
