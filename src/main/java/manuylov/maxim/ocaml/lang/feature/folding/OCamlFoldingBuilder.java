...
public class OCamlFoldingBuilder implements FoldingBuilder
{
	...
	private void appendDescriptors(@Nonnull final ASTNode node, @Nonnull final List<FoldingDescriptor> descriptors)
	{
		...
		if(node.getElementType() == OCamlElementTypes.PARENTHESES_EXPRESSION && firstChildNode != null && firstChildNode.getElementType() ==
				OCamlTokenTypes.BEGIN_KEYWORD || node.getElementType() == OCamlElementTypes.STRUCT_END_MODULE_EXPRESSION || node.getElementType() ==
				OCamlElementTypes.SIG_END_MODULE_TYPE || node.getElementType() == OCamlElementTypes.OBJECT_CLASS_BODY_END_EXPRESSION || node.getElementType() ==
				OCamlElementTypes.OBJECT_END_CLASS_BODY_TYPE || node.getElementType() == OCamlElementTypes.OBJECT_END_CLASS_EXPRESSION || node.getElementType()
				== OCamlElementTypes.COMMENT_BLOCK || node.getElementType() == OCamlElementTypes.UNCLOSED_COMMENT)
		{
