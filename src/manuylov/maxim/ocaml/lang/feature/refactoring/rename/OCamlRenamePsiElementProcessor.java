...
public class OCamlRenamePsiElementProcessor extends RenamePsiElementProcessor
{
			...
			String> conflicts)
	{
		...
		for(final PsiReference referenceBefore : referencesBefore)
		{
			...
			if(referenceElementBefore == null)
			{
				continue;
			}
			...
			if(definitionAfter == null)
			{
				continue;
			}
			if(!rename(referenceElementAfter, newName))
			{
				continue;
			}
			...
			{
				if(!rename(definitionAfter, newName))
				{
					continue;
				}
			...
			else
			{
				...
				if(!(definitionFakeFile instanceof OCamlFile))
				{
					continue;
				}
				actualDefinitionAfter = OCamlResolvingUtil.resolveWithFakeModules(referenceAfter, (OCamlFile) definitionFakeFile);
			}
			if(actualDefinitionAfter == null)
			{
				continue;
			}
			if(definitionAfter != actualDefinitionAfter)
			{
				if(!rename(referenceElementAfter, oldName))
				{
					continue;
				}
				if(!rename(definitionAfter, oldName))
				{
					continue;
				}
				addConflictIfNeeded(conflicts, actualDefinitionAfter, "Warning: %s is already defined.", Pair.create(referenceFakeFile, referenceOriginalFile),
						Pair.create(definitionFakeFile, definitionOriginalFile));
			}
		}

		final PsiElement elementAfter = OCamlPsiUtil.copy(element);
		if(!rename(elementAfter, newName))
		{
			return;
		}
		...
		if(!rename(elementAfter, oldName))
		{
			return;
		}
...
/*
		final Collection<OCamlModule> modules = fileModule.collectAllDependenciesIgnoringCycles(); //todo fix it: should be not dependencies but
        depends on
