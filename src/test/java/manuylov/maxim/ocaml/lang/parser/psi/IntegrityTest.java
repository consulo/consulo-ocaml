...
public class IntegrityTest extends BaseOCamlTestCase
{
	...
	public void testVisitorMethodsHasCorrectParameters() throws IllegalAccessException, ClassNotFoundException
	{
		...
		for(final Method method : methods)
		{
			...
			assertTrue(method.getName() + ", " + name, name.startsWith("OCaml"));
	...
	{
		Method endsCorrectly = BaseOCamlElement.class.getMethod("endsCorrectly");
		...
		processElementTypes(new ElementTypeProcessor()
		{
			public void process(@Nonnull final IElementType type, @Nonnull final OCamlElement element)
			{
				...
				if(name.endsWith("NameImpl"))
				{
					return;
				}
				if(name.endsWith("NameDefinitionImpl"))
				{
					return;
				}
				if(name.endsWith("NamePatternImpl"))
				{
					return;
				}
				if(name.endsWith("NameExpressionImpl"))
				{
					return;
				}
				if(name.startsWith("OCamlFile"))
				{
					return;
				}
				if(name.startsWith("OCamlUnderscore"))
				{
					return;
				}
				while(clazz != null && !clazz.equals(BaseOCamlElement.class))
				{
					if(hasDeclaredMethod(clazz, "endsCorrectly"))
					{
						return;
					}
